...
export const CHANGELOG_DATA: ChangelogEntry[] = [
  {
    ...
    "blocks": [
      ...
      {
        "kind": "bullet",
        "text": "抬头图、条码、码下数字、页数提示一并消失：分页脚本要把手机端的排版算法搬进测量 WebView 执行，此前是按名逐个搬函数，而算法内部调用的一个私有辅助函数没被一起搬过去。它在定时器回调里抛\"未定义\"错误——那已经跳出了脚本自己的错误处理，所以既不报错也不回传结果，预览于是永远停在测量阶段，这几样全都不出现。现在整套算法收进一个自包含函数，结构上不再可能\"忘了带上谁\"，并加了一条把注入函数抠出来独立执行的回归检查。"
      },
      {
        "kind": "bullet",
      ...
      {
        "kind": "bullet",
        "text": "分页落点仍受字体影响：用真实作业纸量到整份正文高 938 CSS px，而一页版心是 933.5px，只差 4.5px。电脑版用宋体排版时行高更大、断点自然更靠前，因此页数与断点在换字体的边际上仍可能与电脑版差一页——这与分页算法无关，取决于中文宋体是否内置。"
      },
      {
        "kind": "bullet",
        "text": "周次能否显示取决于服务器是否给学期起止日期或周次字段；新增的\"日程JSON\"导出可以一次确认。"
      }
    ]
  },
  {
