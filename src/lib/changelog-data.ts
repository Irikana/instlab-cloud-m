...
export const CHANGELOG_DATA: ChangelogEntry[] = [
  {
    ...
    "blocks": [
      ...
      {
        "kind": "bullet",
        "text": "抬头图、条码、码下数字、页数一并消失：分页脚本要把手机端的排版算法搬到测量 WebView 里执行，此前是按名逐个搬函数，而算法内部调用的一个私有辅助函数没被一起搬过去。它在定时器回调里抛\"未定义\"错误，既进不了脚本自己的错误处理、也就不会回传任何结果，于是预览永远停在测量阶段——抬头图、条码、码下数字、页数提示全都出不来。现在整套算法收进一个自包含函数，结构上不再可能\"忘了带上谁\"。"
      },
      {
        "kind": "bullet",
        "text": "超过一页高度的题目会被裁掉：同上一个问题相关——整块挪页时，比一页还高的块会被页面容器直接裁切，内容凭空少一截。现在一路向下拆到装得下为止（真实作业纸是 表格 > 表体 > 行 > 单元格 的层层嵌套，中间每层往往只有一个孩子），并把同一页里同源的相邻片段并回同一个外壳，表格不会被拆成两张。"
      },
      {
        "kind": "bullet",
      ...
      {
        "kind": "bullet",
        "text": "分页落点还受字体影响：用真实作业纸量到整份正文高 938 CSS px，而一页版心是 933.5px —— 只差 4.5px。电脑版用宋体排版时行高更大，断点自然更靠前。因此页数与断点在换字体的边际上仍可能与电脑版差一页，这与分页算法无关，取决于下一条提到的宋体是否内置。"
      },
      {
        "kind": "bullet",
